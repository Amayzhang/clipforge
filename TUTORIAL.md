...
点了免费快剪后它会自动跑：**判官团过台词（自动重写弱句，不花钱）→ 配画面（免费素材，优先实拍视频、配不到退图片）→ 免费配音 + 字幕 → 本机合成**。通常 1~3 分钟。自动配画面只会保留贴题或主题相关的素材；如果某个镜头搜不到足够相关的素材，会明确标出镜头编号并留空，方便你改检索词或上传自己的画面，不会静默塞入抽象背景。
...
| 火山引擎报 `unable to decode '' as int64` | 旧设置把空的 Seed 保存成了字符串，或图片编辑请求缺少正确的多图字段 | 更新到最新版后重试；设置 → 生图模型选择 `doubao-seedream-4-0-250828` 或已开通的 Seedream 模型。九宫格会自动传入主播定妆图/商品图；如果仍失败，先点「测试连接」确认 Key 和模型权限 |
| 九宫格按钮不可用 | 当前脚本少于 2 个或多于 9 个镜头，或导演模式未打开 | 设置/侧边栏切到「导演模式」；脚本控制在 2–9 个镜头，先生成脚本再进入「素材」页操作 |
| 自动配画面提示有镜头留空 | 免费库没有找到足够相关的候选 | 修改该镜头的英文检索词，或在素材卡点「上传素材」；留空是为了避免成片混入无关背景 |
